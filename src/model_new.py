...
class CIFAR10ProCNN(nn.Module):
    """
    Advanced Deep CNN architecture optimized for CIFAR-10.

    Key features:
    - 8 Convolutional layers organized in 4 VGG-style blocks.
    - Progressive Dropout (0.2 to 0.5) to prevent overfitting.
    - Extensive use of Batch Normalization for training stability.
    """

    def __init__(self, n_classes: int = 10) -> None:
        super(CIFAR10ProCNN, self).__init__()

        def conv_block(in_f: int, out_f: int, dropout_rate: float) -> nn.Sequential:
        ...
        # 4 Blocks with increasing filter counts
